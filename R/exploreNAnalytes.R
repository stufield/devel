...
#' 
#' Trains a model on the data, tests errors
#' 
...
#' @param order A string indicating how to order the analytes as they are added to the model
#'   If NULL, the order in \code{somamers} is used. 
#'   By default ("response"), it is KS-distance for binary responses and 
#'   Spearman correlation with response for continuous ones
...
exploreNAnalytes <- function(
  ...
  if (!is.null(order) && !(order %in% c("response"))) {
    stop("order should be either NULL or 'response'")
  }
  ...
  if (is.null(somamers)) {
    ...
    N <- somamers
    somamers <- order_somamers(snames[1:N], adat, response)
    order <- NULL
  ...
  if (!is.null(order) && order=="response") {
    somamers <- order_somamers(somamers, adat, response)
  }
  # now restrict
...
#' 
#' @return a list of somamer names, same as soma_list, but ordered according to KS distance (binary response)
#'   or by Spearman correlation to resposne (continuous)
order_somamers <- function(soma_list, adat, response) {
  ...
  if (is.factor(adat[[response]])) {
    ...
    tmp_D <- sapply(soma_list, function(sn) as.numeric(ks.test(tmp_l[[1]][,sn], 
                                                                 tmp_l[[2]][,sn])$statistic))
  } else if (is.numeric(adat[[response]])) {
    # use spearman correlations
    tmp_D <- sapply(soma_list, function(sn) cor(adat[,sn], adat[[response]], method = "spearman"))
   } else {
    # bail out
