...
#' @return accept_limit the cutoff values for the experiment
#' @return prob_bad a probability value (a scalar) for at least one of the metrics to eventually reach production limits 
#' at least prod_limit_fraction of the times. 
#' @return min_shift the minimum shifts we can expect to detect, with (overall) probability power
#' ie: if the shift is less than this, the probability of rejecting at least one of them is at least power
#' @return min_shift_bad the minimum shifts we can accept until we see production rejections 
#' of about prod_limit_fraction of the runs (this depends on Xone and prod_limit and prod_limit_fraction, 
#' not acceptance_limit)
#' @return min_n_bad the number of times a test like this should produce typeII errors of size min_shift until we reach 
#' production rejection troubles: (log(min_shift)/log(accept_limit) for scaled)
#' @return prob_n_bad probability of min_n_bad happening (1-power)^(min_n_bad)
#' @return typeI1 per-metric/batch typeI error (so that in the end we have the given typeI)
#' @return beta1 per-mmetric/batch power (so that in the end we have the given power)
#' 
#' Note that one can use 'summary' on the return list to obtain a table of all the accept_limit, min_shift and prob_n_bad 
#' and the prob_bad values in a nicer format
#' 
#' @example 
